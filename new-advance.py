    ...
    if os.path.exists('/proc/device-tree/model'):
        with open('/proc/device-tree/model', 'r') as f:
            ...
            if 'Raspberry Pi' in model:
                ...
                print("Running on a Linux system that is not Raspberry Pi.")
    else:
        from torchvision.models import resnet101, ResNet101_Weights
        print("Device tree model file not found. Assuming a non-Raspberry Pi Linux system.")
...
class CalibratedGazeTracker:
        ...
        if self.system == "Windows":
            ...
            if os.path.exists('/proc/device-tree/model'):
                with open('/proc/device-tree/model', 'r') as f:
                    ...
                    if 'Raspberry Pi' in model:
                        self.cap = Picamera2()
                        preview_config = self.cap.create_preview_configuration(main={"size": (1920, 1080)})  # 1920x1080
                        ...
                        print("Running on a Linux system that is not Raspberry Pi.")
                        self.cap = cv2.VideoCapture(0)  # Example for a USB camera
                        self.screen_width = 1280  # Adjust according to your camera
    ...
    def calibrate(self):
        ...
        for i, (rel_x, rel_y) in enumerate(self.calibration_points):
            ...
            if eye_features:
                ...
                print(f"Recorded calibration point{i} at {timestamp}")
    ...
    def get_gaze_point(self, frame):
        ...
        if left_eye is not None and right_eye is not None:
            ...
            # Remove the inversion for the x-coordinate           #
            # x = int(np.clip((gaze[0] + 1) / 2 * self.screen_width, 0, self.screen_width - 1))
            # y = int(np.clip((gaze[1] + 1) / 2 * self.screen_height, 0, self.screen_height - 1))1))

            # Remove the inversion for the x-coordinate            x = int(np.clip((gaze[0] + 1) / 2 * self.screen_width, 0, self.screen_width - 1))
    ...
    def record_gaze_data(self, duration_seconds):
        ...
        while True:
            ...
            if gaze_point is not None:
                ...
                print(f"Gaze point recorded at {timestamp}")
    ...
    def generate_heatmap(self, gaze_data):
        ...
        if platform.system() == "Windows":
            ...
            plt.imshow(heatmap, cmap='jet', alpha=0.6, origin='upper', vmin=0, vmax=255)
            plt.axis('off')
...
def main():
    ...
    try:
        tracker.calibrate()
        tracker.record_gaze_data(duration_seconds=10)
