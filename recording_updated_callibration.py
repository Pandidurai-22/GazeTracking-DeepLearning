...
class GazeRecorder:
    def __init__(self):
        ...
        from calibrated_gaze_tracker import GazeEstimationModel
        self.gaze_model = GazeEstimationModel()
        
        # Try loading custom model first, then ETH-XGaze
        custom_model_path = 'best_custom_gaze_model.pth'
        eth_model_path = 'eth-xgaze_resnet18.pth'
        
        if os.path.exists(custom_model_path):
            checkpoint = torch.load(custom_model_path, map_location=self.device)
            self.gaze_model.model.load_state_dict(checkpoint['model_state_dict'])
            print("Loaded custom trained gaze model!")
        elif os.path.exists(eth_model_path):
            state_dict = torch.load(eth_model_path, map_location=self.device)
