...
class GazeEstimationModel(torch.nn.Module):
    def __init__(self):
        ...
        if platform.system() == "Windows":
            ...
            self.model = EfficientNet.from_pretrained('efficientnet-b7')  # Choose the version you prefer
            self.model._fc = torch.nn.Linear(self.model._fc.in_features, 2)  # Adjust the final layer for your output
        else:
            self.model = EfficientNet.from_name('efficientnet-b7', num_classes=2)
            self.model.fc = torch.nn.Linear(2048, 2)
...
class CalibratedGazeTracker:
    def __init__(self):
        ...
        if self.system == "Windows":
            ...
            self.cap = Picamera2()
            # Set optimized parameters for Linux/Raspberry Pi
        ...
        # Load model weights
        custom_model_path = 'best_custom_gaze_modelb7.pth'
        eth_model_path = 'efficientnet_b7_lukemelas-dcc49843.pth'
